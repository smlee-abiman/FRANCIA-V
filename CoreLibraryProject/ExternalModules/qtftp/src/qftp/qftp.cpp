...
#define ENABLE_QFTPDTP_PARSE_DIR_EXT    // extension parse server style
...
#ifdef ENABLE_QFTPDTP_PARSE_DIR_EXT
    // DOS style FTP servers(Extension:Adjust the time display to the server side)
    QRegExp dosPatternExt(QLatin1String("^(\\d\\d-\\d\\d-\\d\\d\\d\\d \\ \\d\\d:\\d\\d:\\d\\d)\\s+"
                                     "(<DIR>|\\d+)\\s+(\\S.*)$"));
    if (dosPatternExt.indexIn(bufferStr) == 0) {
        _q_parseDosDir(dosPatternExt.capturedTexts(), userName, info);
        return true;
    }
#endif // ENABLE_QFTPDTP_PARSE_DIR_EXT
    // Unsupported
    return false;
}
