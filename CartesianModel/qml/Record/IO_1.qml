...
Item {
    ...
    property int numIoHistoryItem: ioHistoryDataList.length
    ...
    Component.onCompleted: numPages = getNumPages(view_type, numIoHistoryItem)

    onView_typeChanged: numPages = getNumPages(view_type, numIoHistoryItem)

    onIoHistoryDataListChanged: {
        let prevPage = page
        let numItem = ioHistoryDataList.length
        if (numItem === 0) {
            page = 0
        } else {
            if (page === 0) {
    ...
    Component{
        ...
        Rectangle{
            ...
            Text{
                ...
                text :viewModel.toIoChangeString(index)
            }
        }
    }
    ListView{
...
Rectangle {
    ...
    Rectangle {
        ...
        Image {
            anchors.centerIn: parent
            source: (view_type === 0)
                    ? resourceManager.imagePath + "/33_ALM_history/a330_08.png"
        ...
        MouseArea {
            ...
            onClicked: {
                ...
                if (view_type === 0) {
                    view_type = 1
                    if (ioHistoryDataList.length === 0) {
                        page = 0
                    } else {
                        if (page === lastPage) {
                            topItemIndex = Math.max(ioHistoryDataList.length - numListItemsPerPage - 1, 0)
                        } else {
                    ...
                    if (ioHistoryDataList.length === 0) {
                        ...
                        if (page === lastPage) {
                            topItemIndex = Math.max(ioHistoryDataList.length - numDetailItemsPerPage - 1, 0)
                        } else {
    ...
    Loader {
        ...
        sourceComponent: RecordDialog {
            msg: resourceManager.label(50, 32, resourceManager.language)
            msgColor: resourceManager.rgb(50, 32)
    ...
    Loader {
        ...
        onLoaded: item.open()
        sourceComponent: RecordDialog {
            msg: resourceManager.label(50, 187, resourceManager.language)
            msgColor: resourceManager.rgb(50, 187)
            language: resourceManager.language
    ...
    Loader {
        ...
        onLoaded: item.open()
        sourceComponent: RecordDialog {
            msg: resourceManager.label(50, 90, resourceManager.language)
            msgColor: resourceManager.rgb(50, 90)
            language: resourceManager.language
            okButtonVisible: false
            onCancelClicked: close()
        }
