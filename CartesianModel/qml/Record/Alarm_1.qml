...
**************************************************************************************/
Item {
    id: normal_alarm_view
    property var viewModel: alarmHistoryViewModel
    property var alarmList: viewModel.normalAlarmList
    ...
    onPageActiveChanged: {
        if (pageActive) {
            if (!local.isFtpFileAcquired) {
                openAlarmHistoryFileLoadingDialog()
            }
        } else {
            closeAlarmHistoryFileLoadingDialog()
        }
    }
    ...
    function openAlarmHistoryFileLoadingDialog() {
        if (pageActive) {
            alarmHistoryFileLoadingDialog.open()
        }
    }

    function closeAlarmHistoryFileLoadingDialog() {
        if (pageActive) {
            alarmHistoryFileLoadingDialog.close()
        }
    }
    ...
    QtObject {
        ...
        onIsFtpFileAcquiredChanged: {
            if (isFtpFileAcquired) {
                normal_alarm_view.closeAlarmHistoryFileLoadingDialog()
            } else {
                normal_alarm_view.openAlarmHistoryFileLoadingDialog()
            }
        }
    }
...
Rectangle {
    ...
    Component{
        ...
        Rectangle{
            ...
            Text{
                property int alarmNo: modelData.alarmNo * 2 - 1
                width: 420
                height: parent.height
                x:330
                ...
                text: resourceManager.getMessageStringValue("12", alarmNo, local.language)
            }
        }
    }
    ListView{
...
Rectangle {
    ...
    Rectangle {
        ...
        PageText {
            anchors.centerIn: parent
            page: normal_alarm_view.page
            totalPage: normal_alarm_view.numPages
            color: "#666666"
        }
    }
...
Rectangle {
    ...
    Rectangle {
        ...
        Image {
            anchors.centerIn: parent
            source: (view_type === 0) ? resourceManager.imagePath + "/33_ALM_history/a330_06.png" : resourceManager.imagePath + "/33_ALM_history/a330_07.png"
        }
    ...
    Loader {
        id: alarmHistoryFileLoadingDialog
        width: parent.width
        height: parent.height
        ...
        active: normal_alarm_view.pageActive
        asynchronous: true
        visible: (status === Loader.Ready) && normal_alarm_view.pageActive
        onActiveChanged: {
            if (!active) {
                asynchronous = true
            }
        ...
        sourceComponent: RecordDialog {
            msg: resourceManager.label(22, 11, local.language)
    ...
    Connections {
        ...
        enabled: normal_alarm_view.pageActive
