...
Item{
    ...
    QtObject {
        ...
        readonly property var menuModel: menu43Model.subMenuList[1]     // ProgramMenu4_3_2
        readonly property var countType2Addr: menuModel.countType2Addr
        readonly property int selectedCountType2Addr: menuModel.selectedCountType2Addr

        readonly property int numOfItems: countType2Addr.length
        readonly property int numOfItemsParPage: 8
        readonly property int numOfPages: Math.ceil(numOfItems / numOfItemsParPage)
        property int currentPage: 0

        onSelectedCountType2AddrChanged: {
            let index = countType2Addr.indexOf(selectedCountType2Addr)
            gotoPage(index)
        }

        function initialize() {
            currentPage = 0
            menuModel.selectedCountType2Addr = -1
        }
        function prevPage() {
    ...
    Component{
        ...
        Rectangle{
            ...
            width: 310
            height: 47
            readonly property int ioAddress: local.countType2Addr[index]
            readonly property bool isSelected: (ioAddress === local.menuModel.selectedCountType2Addr)
            ...
            CheckBox{
                ...
                onClicked:{
                    local.menuModel.selectedCountType2Addr = isSelected ? -1 : ioAddress
                }
            }
        }
    }
