...
import QtQuick 2.12
import QtQuick.VirtualKeyboard 2.3  // 2.4 is not used." Because the error "input method is not set" occurs.
...
KeyboardStyle {
    ...
    readonly property string fontFamily: "Noto Sans SC Regular"
    readonly property real keyBackgroundMargin: Math.round(3 * scaleHint)
    readonly property real keyContentMargin: Math.round(20 * scaleHint)
    readonly property real keyIconScale: scaleHint * 0.3
    readonly property string resourcePrefix: ""	//"qrc:/QtQuick/VirtualKeyboard/content/styles/3GPendant/"
    ...
    readonly property string keyBaseColor: "#1FA7E4"
    readonly property string keyTextColor: "white"


    keyboardDesignWidth: 968
    keyboardDesignHeight: 382
    keyboardRelativeLeftMargin: 2 / keyboardDesignWidth
    keyboardRelativeRightMargin: 2 / keyboardDesignWidth
    keyboardRelativeTopMargin: 2 / keyboardDesignHeight
    keyboardRelativeBottomMargin: 2 / keyboardDesignHeight
...
//            color: "#1e1b18"
    ...
    spaceKeyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            Text {
                id: spaceKeyText
                text: "SPACE"   //Qt.locale(InputContext.locale).nativeLanguageName
    ...
    popupListDelegate: SelectionListItem {
        ...
        Text {
            ...
            font {
                family: fontFamily
                weight: Font.Normal
                pixelSize: 30   //Qt.inputMethod.cursorRectangle.height * 0.8
            }
            function decorateText(text, wordCompletionLength) {
                if (wordCompletionLength > 0) {
                    return text.slice(0, -wordCompletionLength) + '<u>' + text.slice(-wordCompletionLength) + '</u>'
