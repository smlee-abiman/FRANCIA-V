...
AutoProdViewModel::AutoProdViewModel(ResourceManager *resource, CommunicationThread * comm, CommonViewModel *commonView)
...
void AutoProdViewModel::onFinished(){
    if(m_pageChanging == false && m_modbusManager->getIsPageChanging() == false){
        Enums::Pages currentPage = m_commonView->getCurrentPage();
        bool isProductionPage =
            (currentPage == Enums::Pages::AutoProductMngTimer) ||
            (currentPage == Enums::Pages::AutoProductMngCounter);
